  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    if ((isDraggingLeft || isDraggingRight) && rulerRef.current) {
      ...
      if (container) {
        ...
        if (isDraggingLeft) {
          const maxLeftPosition = PAGE_WIDTH - rightMargin - MINIMUM_PAGE_WIDTH;
          const newLeftPosition = Math.min(rawPosition, maxLeftPosition);
          setLeftMargin(newLeftPosition);
        } else {
          const maxRightPosition = PAGE_WIDTH - leftMargin - MINIMUM_PAGE_WIDTH;
          ...
          <div className="relative h-full w-[816px]">
            {markers.map((marker) => (
              <div
                ...
                style={{ left: `${marker * 10}px` }}
              >
                {marker % 10 === 0 && (
                  <>
                    <div className="absolute bottom-0 h-2 w-px bg-neutral-400" />
                ...
                {marker % 5 === 0 && marker % 10 !== 0 && (
                  <>
                    <div className="absolute bottom-0 h-1.5 w-px bg-neutral-400" />
                  </>
                )}
                {marker % 5 !== 0 && (
                  <>
